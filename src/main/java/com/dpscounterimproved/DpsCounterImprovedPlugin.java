...
@Slf4j
@PluginDescriptor(
	name = "Dps Counter Improved"
)
public class DpsCounterImprovedPlugin extends Plugin
{
	...
	@Inject
	private DpsCounterImprovedOverlay dpsCounterImprovedOverlay;

	@Inject
	private DpsCounterImprovedConfig dpsCounterImprovedConfig;
	...
	@Provides
	DpsCounterImprovedConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(DpsCounterImprovedConfig.class);
	}
	...
	{
		total.reset();
		overlayManager.add(dpsCounterImprovedOverlay);
	...
	protected void shutDown()
	{
		...
		overlayManager.remove(dpsCounterImprovedOverlay);
		members.clear();
	}
	...
	public void onHitsplatApplied(HitsplatApplied hitsplatApplied)
	{
		...
		if (hitsplat.isMine())
		{
			...
			if (dpsCounterImprovedConfig.bossDamage() && !isBoss)
			{
				return;
			}
		...
		{
			if ((dpsCounterImprovedConfig.bossDamage() || actor != player.getInteracting()) && !isBoss)
			{
	...
	public void onDpsUpdate(DpsUpdate dpsUpdate)
	{
		...
		if (!dpsUpdate.isBoss() && dpsCounterImprovedConfig.bossDamage())
		{
			return;
		}
	...
	public void onNpcDespawned(NpcDespawned npcDespawned)
	{
		...
		if (npc.isDead() && BOSSES.contains(npc.getId()))
		{
			...
			if (dpsCounterImprovedConfig.autoreset())
			{
				members.values().forEach(DpsMember::reset);
				total.reset();
			}
			else if (dpsCounterImprovedConfig.autopause())
			{
				pause();
			}
		}
	...
	void pause()
	{
		...
		dpsCounterImprovedOverlay.setPaused(true);
	}
	...
	void unpause()
	{
		...
		dpsCounterImprovedOverlay.setPaused(false);
	}
}